...
公开站点通过 `site/publish.manifest.json` 默认拒绝发布，只同步经过审核的公司档案、证据元数据、强关系和显式允许的图片到独立 Cloudflare D1/R2。`note`、内部方法、弱 body mentions、低质量 source 和未审核资产不进入公开投影；Memex 本地 vault 仍是唯一写入源。
