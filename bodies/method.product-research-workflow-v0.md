...
Hyperagent 这次不是“搜不够”的问题，而是暴露了调研收尾和证据状态管理的问题：Product Hunt 一开始返回安全验证，后来可以打开；Similarweb 公网页显示无数据，但中转站能看到完整流量。方法上要把“抓取失败”和“源本身无数据”分开。
...
流量、渠道、地域、相似站点、关键词这些判断，优先用中转站 Similarweb。公开 Similarweb 页只适合兜底，不能用来得出“没有数据”的结论。
...
- Similarweb、Google Trends、Product Hunt、LinkedIn 的采集方式还需要继续产品化；其中 Similarweb 已明确应优先走中转站。
