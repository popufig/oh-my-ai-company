...
## 2026-07-20 Maisa 调研补充

本轮对象：[[company.maisa]]。产品判断：[[note.maisa-takeaway-2026-07-20]]；过程记录：[[note.maisa-research-run-2026-07-20]]。

回看 Paperclip、Relevance AI、Ema 与 Evose 的治理证据后，Maisa 只增加一项“可靠 Agent”主张的核验方式，不把单个厂商的架构叙事升级为通用结论。

### 把 deterministic 拆成四层证据

1. **Planning**：同一输入是否产生相同计划，还是由模型动态决定步骤；
2. **Execution**：生成后的代码、API 调用和工具动作是否可复现、可限权；
3. **Validation**：每一步由什么规则、数据或人工确认，失败如何重试、升级和停止；
4. **Outcome**：最终交付是否被业务接受，错误率、人工介入、回滚、成本和留存是否有证据。

代码执行可复现，不能自动证明动态规划确定；步骤有 validation，也不能自动证明端到端无幻觉。厂商使用 deterministic、hallucination-resistant、no false positives 等词时，应分别记录其作用层和证据来源。

### 把执行收据与业务验收分开

[[concept.agent-execution-receipt]] 记录目标、步骤、工具、来源、校验、异常和结果，能加强追踪、审计与复盘。但“留下完整 Chain of Work”只说明过程可检查，不等于结果已经正确、被客户接受或可以安全回滚。后续同类产品同时追 execution receipt 与 accepted outcome，不能用前者替代后者。

## 2026-07-21 CoreSpeed 调研补充

本轮对象：[[company.corespeed]]。过程记录：[[note.corespeed-research-run-2026-07-21]]。

回看 Marblism/Motion-Wonderly 的 product genealogy、Lindy 的前台/底座双读、Pancake 的主体连续性和 Evose 的 enterprise claims audit 后，本轮只补两项现有方法还未明确覆盖的动作。

### Pivot 公司增加 lineage matrix

当公司明确宣布 new direction 时，不把全部历史证据自动迁移到当前产品。固定拆成四列：

1. 历史产品与当时的购买对象；
2. 当前产品与新的核心控制点；
3. 可以继承的资产，例如团队、代码、主体和通用基础设施；
4. 不可继承的结论，例如客户、采用、流量、定价、认证和功能 shipped state。

CoreSpeed 的 Zypher 仓库能证明团队持续交付 Agent framework/runtime，不能证明新的 1,000+ connectors、portable memory、Agent Pay 或 One Bill 已上线。只有机制连续且有当前证据时，历史资产才进入新产品能力或采用判断。

### Access/authorization 产品增加八列验收

只写“OAuth + secure policy”会掩盖真正的权限和责任边界。对 access layer、MCP runtime、connector/auth platform 固定核对：

1. connector coverage 与 action coverage；
2. credential storage、encryption 与 tenant isolation；
3. scope、consent、revoke 与 offboarding；
4. policy precedence、standing authorization 与 classifier 误判；
5. audit、escalation、rollback 与 incident handling；
6. memory ownership、lineage、export 与 delete；
7. payment authorization、custody、refund/chargeback 与 liability；
8. billing meter、第三方转售、预算与 cost per accepted action。

官网 logo、动画账单和 FAQ 只证明厂商设计声明。至少出现公开 docs、账号内 smoke、客户侧证据或采购材料之一，才能把对应能力从 candidate 升级为 shipped；多列一致后才可写 production-ready。

本轮还确认一个工具纪律：同一 browser tab 的 selector screenshot 串行执行。并发失败属于自动化生命周期问题，不应混入产品可用性结论。

## 2026-07-21 Sarea 调研补充

本轮对象：[[company.sarea]]，并与运营主体 [[company.corespeed]] 做产品谱系对照。过程记录：[[note.sarea-research-run-2026-07-21]]。

回看 CoreSpeed 的 pivot/lineage matrix、Kylon 的实现/采用双链、Pancake 的 legal operator 和 Floatboat 的近名消歧后，本轮只补两个此前没有明确写入的检查。

### 可下载桌面产品建立 package receipt

对 macOS/Windows 桌面产品，landing page 和 release screenshot 之外，固定记录：

1. download redirect、版本、文件大小与内容类型；
2. cryptographic hash、bundle/package ID；
3. CPU architectures、最低系统与关键 permission description；
4. Developer ID/code signing、hardened runtime 与 entitlements；
5. notarization/Gatekeeper 或 Windows reputation 的实际可验证边界；
6. bundled agents/connectors/tools/skills 与 updater；
7. 最小启动 smoke、崩溃和后台进程；
8. 静态包、未登录启动、登录后 feature smoke 三层证据分别总结。

安装包存在且能启动，只能升级“已交付客户端”，不能自动升级 connector 成功率、后台 workflow、wallet、采用或长期稳定性。包内 fixture/test snapshot 证明实现路径被覆盖过，不等于生产 outcome。

### 条件式商业条款增加 language gate

Terms、FAQ 或定价出现 `may offer`、`when activated`、`if available`、`planned`、`coming soon` 时，对应能力只标为设计/准备状态。官网的确定性 UI、示例余额和价格不能覆盖法律文本里的条件。

支付、wallet、credits 或 outcome billing 至少需要账号内入口、真实小额交易/账单、processor/ledger 条款或客户侧 receipt 之一，才可升级为 live；退款期限只有在充值入口真实存在时才说明商业机制已经运行。
